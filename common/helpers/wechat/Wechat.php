...
use backend\models\WechatToken;
...
class Wechat
{
    ...
    public function getToken($flag = false)
    {
        ...
        if (!$token || $flag) {
            ...
            Yii::info($url, 'yii\db\Command::query');
            $data = RequestHelper::requestGet($url);
            Yii::info($data, 'yii\db\Command::query');
            $data_arr = json_decode($data, true);
            $token = $data_arr["access_token"];
            Yii::$app->cache->set('token', $token, 6000);
            Yii::info('重新获取token', 'yii\db\Command::query');
        }
     ...
     * @return mixed
     */
    public function getTicket($id, $options, $again = false)
    {
        $url = 'https://api.weixin.qq.com/cgi-bin/qrcode/create?access_token=' . $this->getToken($again);
        Yii::info($url, 'yii\db\Command::query');
        $sendData = [
        ...
        $request = RequestHelper::requestPost($url, json_encode($sendData));

        Yii::info($request, 'yii\db\Command::query');
        $res_decode = json_decode($request);
        if (!isset($res_decode->ticket)) {
            return $this->getTicket($id, $options, true);
        } else {
            $ticket = $res_decode->ticket;
        }
     ...
     * -v 27
     */
    public function getQrByTicket($ticket, $name)
    {
        ...
        $file_name = RequestHelper::requestPost($config['upload'], $data);
        $res = json_decode($file_name);
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    private function http_get($url)
    {
        ...
        Yii::info($sContent, 'agent\wxToken');
        curl_close($oCurl);
